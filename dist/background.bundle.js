...
/******/ 	return __webpack_require__(__webpack_require__.s = "./background.main.js");
/******/ })
/************************************************************************/
/******/ ({
...
var processFlow = __webpack_require__(/*! ./background/processFlow.js */ "./background/processFlow.js");
var constants = __webpack_require__(/*! ./background/constants.js */ "./background/constants.js");
...
var constants = __webpack_require__(/*! ./constants */ "./background/constants.js");

module.exports = {

  sendMsg: function sendMsg(tabId, json, callback) {
    chrome.tabs.query({ currentWindow: true }, function () {
      chrome.tabs.sendMessage(tabId, json, function () {
  ...
  sendCommand: function sendCommand(tabId, command, callback) {
    setTimeout(function () {
      module.exports.sendMsg(tabId, { command: command, action: 'run_cmd' }, callback);
    }, constants.DELAY_AFTER_INJECT);
  },

  inject: function inject(tabId, fileNames, callback) {
  ...
  open: function open(tabId, url, callback) {
  ...
  newTab: function newTab(url, callback) {
    if (!url) {
  ...
  getUrl: function getUrl(base, path) {
    if (!path) {
      return null;
    } else {
      return base + path;
  ...
  executeScripts: function executeScripts(tabId, fileNames, finalCallback) {
    ...
    var nestedCallback = function nestedCallback() {
      return finalCallback(tabId);
    };

    for (var i = fileNames.length - 1; i >= 0; --i) {
      nestedCallback = createCallback(tabId, fileNames[i], nestedCallback);
    }if (nestedCallback !== null) nestedCallback(); // execute outermost function
  },

  isOfDomain: function isOfDomain(url, base) {
...
module.exports = {
    getAppInfo: function getAppInfo(appKey) {
        return new Promise(function (resolve) {
            $.getJSON('./sample_data/angular-app.json').then(function (data) {
                return resolve(data[appKey]);
            });
        });
    },

    // NOTE same as getAppInfo
    getUserInfo: function getUserInfo(userKey) {
        // Of course we're not gonna get the entire user base in reality, ha, ha.
        return new Promise(function (resolve) {
            $.getJSON('./sample_data/user.json').then(function (data) {
                return resolve(data[userKey]);
            });
        });
    },

    getLoginInfo: function getLoginInfo(appKey, auth) {
        console.log("Getting login info...");
        return new Promise(function (resolve) {
            $.getJSON('./sample_data/angular-app.json', function (data) {
...
var constants = __webpack_require__(/*! ./constants.js */ "./background/constants.js");
...
exports.processForms = function (dataArr, appAuth, skipLogin) {
    ...
    var _dataArr = _slicedToArray(dataArr, 2),
        app = _dataArr[0],
        user = _dataArr[1];
    ...
    var info = _jquery2.default.extend({}, app);
...
function handleProcesses(info, user, process, endOfLogin) {
    ...
    for (var i = 0; i < endOfLogin; ++i) {
        handleCommand(info, user, process[i], 'bl');
    }
    for (var _i = endOfLogin; _i < pLen; ++_i) {
        handleCommand(info, user, process[_i]);
    }
}

function handleCommand(info, user, cmd, imp) {
    if (typeof cmd === 'string') handleOneProcess(info, user, cmd, imp);else handleConditional(info, user, cmd);
}
    ...
    if (conditionals[0].try) {
        ...
        for (var i = 0; i < conditionals.length; ++i) {
            ...
            if (c.try) {
                ...
                console.assert(i === conditionals.length - 1, "Conditional without assertions is not" + "the last conditional");
                break;
            }
        }
        ...
        for (var _i2 = 0; _i2 < conditionals.length; ++_i2) {
            c = conditionals[_i2];
...
function meetsReq(user, req) {
    ...
    var len = req.length;
    for (var i = 0; i < len; ++i) {
        if (!getVal(user, req[i])) return false;
    }
    return true;
}

function splitCommand(c) {
    var _c$split = c.split(constants.OPT_SEP),
...
function getVal(user, userKey) {
    ...
    if (userKey.includes(constants.SEL_SEP)) {
        var _userKey$split = userKey.split(constants.SEL_SEP),
            _userKey$split2 = _slicedToArray(_userKey$split, 2),
            fieldKey = _userKey$split2[0],
            optionVal = _userKey$split2[1];

        return getUserVal(user, fieldKey) === optionVal ? fieldKey : null;
    } else {
        if (userKey.includes(constants.FORMAT_IND)) {
            var _userKey$split3 = userKey.split(constants.FORMAT_IND),
                _userKey$split4 = _slicedToArray(_userKey$split3, 2),
                newKey = _userKey$split4[0],
                format = _userKey$split4[1];
...
var _session = __webpack_require__(/*! ./session.js */ "./background/session.js");
...
exports.startProcesses = function (appKey, userKey, auth, skipLogin) {
  ...
  new _session2.default(processComplete, processFailure).start(appKey, userKey, auth, skipLogin);
};
...
function processFailure(sum, reason, implication) {
  ...
  console.log(sum);
  console.log("WARNING: Since the process failed, the last few field values may not" + "have been sent.");
  processCleanup();
}
...
/* global chrome */
var infoLoad = __webpack_require__(/*! ./infoLoad.js */ "./background/infoLoad.js");
var helpers = __webpack_require__(/*! ./helpers.js */ "./background/helpers.js");
var parser = __webpack_require__(/*! ./parser.js */ "./background/parser.js");
var constants = __webpack_require__(/*! ./constants.js */ "./background/constants.js");
...
var Session = function () {
	...
	_createClass(Session, [{
		key: 'start',
		value: function start(appKey, userKey, auth, skipLogin) {
			var _this = this;

			Promise.all([infoLoad.getAppInfo(appKey), infoLoad.getUserInfo(userKey)]).catch(function () {
				return _this.failure('get_info');
			}).then(function (data) {
				return parser.processForms(data, auth, skipLogin);
			}).then(function (info) {
				return _this.runFormSeq(info);
			});
		}
	}, {
		key: 'getSummary',
		...
		key: 'runFormSeq',
		value: function runFormSeq(info) {
			this.info = info;
			this.idx = 0;
		...
		key: 'startCheckingTimeout',
		value: function startCheckingTimeout() {
			var _this2 = this;

			this.lastTimeActive = Date.now();
			var end = function end() {
				return _this2.failure.call(_this2, 'timeout');
			};
		...
		value: function getMessageHandler() {
			if (!this.actionMessageHandler) this.actionMessageHandler = this.onMessage.bind(this);
			return this.actionMessageHandler;
		}
	}, {
		key: 'getTabLoadHandler',
		value: function getTabLoadHandler() {
			if (!this.tabLoadHandler) this.tabLoadHandler = this.onPageLoad.bind(this);
   ...
   */
		...
		key: 'onPageLoad',
		value: function onPageLoad(tabId, changeInfo) {
		...
		value: function onMessage(request) {
			...
			switch (request.state) {
				...
				/* falls through */
				case 'next':
					if (this.pendingField) this.completedFields.push(this.pendingField);
				/* falls through */
				case 'try_unmet':
					if (this.waitingFor === WaitingFor.MESSAGE) {
						this.runNextCommandWithDelay();
					} else {
						console.error("Expected waitingFor to be MESSAGE but got " + this.waitingFor + " instead.");
					}
					break;
				case 'injected':
					this.runNextCommandWithDelay();
		...
		key: 'runNextCommand',
		value: function runNextCommand() {
			var _this3 = this;

			if (this.idx >= this.info.process.length && this.queue.length === 0 && this.conditionals.length === 0) {
			...
			if (cmd.action) console.log('Running command ' + cmd.action);else this.failure('Command has no action: ' + JSON.stringify(cmd));
			...
			switch (cmd.action) {
				case 'open':
					...
					if (this.idx === 1 && this.queue.length === 0) {
						...
						helpers.newTab(this.getFullUrl(cmd.target), function (tabId) {
							_this3.tabId = tabId;
						});
					} else {
			...
			if (cmd.flag === 'n') this.waitingFor = WaitingFor.NEW_PAGE_LOAD;

			this.lastTimeActive = Date.now();
		}
	}, {
		key: 'runNextCommandWithDelay',
		value: function runNextCommandWithDelay() {
			var _this4 = this;

			setTimeout(function () {
				return _this4.runNextCommand();
		...
		value: function getNextCommand() {
			if (this.conditionals && this.conditionals.length > 0) {
				...
				if (this.queue.length === 1) this.idx++;
